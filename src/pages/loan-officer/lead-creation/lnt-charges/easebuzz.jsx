...
// drop-down changes
  ...
  const [amount, setAmount] = useState(1500);4
  ...
  useEffect(()=> {
    ...
    editFieldsById(values?.lead?.id,'lead',{
        ...
        selected_gateway:'easebuzz-payment'
    }
    ,{
        headers: {
          Authorization: token,
      ...
      setFieldValue('lead.selected_gateway', 'easebuzz-payment');
 ...
 const resps = await addLnTCharges(values?.lead?.id, {
      ...
      for (var i=0;i<values?.applicants.length;i++){
        if(values?.applicants[i].applicant_details.applicant_type=='Primary Applicant' && values?.applicants[i].applicant_details.lead_id==values?.lead?.id){
          ...
          try{
            ...
            console.log("ERROR REGENRATING BRE 101 Easebuzz page",err)
          }
    ...
    .then((blob) => {
      ...
      reader.onloadend = () => {
        ...
        if (typeof result === "string") {
          ...
          console.error("Failed to read image as Base64 string.");
        }
      };
      reader.readAsDataURL(blob);
    })
      }//setQrCode(resp.DecryptedData);
    } catch (err) {
      console.log(err);
    } finally {
      setLoadingQr(false);
  ...
  useEffect(()=> {
    ...
    // check if LNT Exists on mounting if no then only process with the QR code to avoid unnecessary order creation if already paid ** 25/01
      ...
      (async () => {
        try {
          ...
          (async function fetchCode() {
            ...
            const resp = await addLnTCharges(body, {
              headers: {
                Authorization: token,
              },
            },bypass);
  ...
  const handleCheckingStatus = async (label = '') => {
    ...
    if(values?.lead?.id == undefined) {
        return;
    }

    try {
      setCheckingStatus(label);
      const resp = await checkPaymentStatus(values?.lead?.id, {gateway:'easebuzz'},{
        headers: {
          Authorization: token,
        },
      });
        ...
        <>
          <div className='overflow-hidden flex flex-col h-[100vh] bg-medium-grey'>
            ...
            {loadingPaymentStatus ? (
              ...
              <>
              <ModalComponent open={startLoader}/>

...
// import { editDoc, editFieldsById, getApplicantById, IMAGE_URL_FOR_PDF_UPLOAD } from '../../global';
// import { LeadContext } from '../../context/LeadContextProvider';
// import { AuthContext } from '../../context/AuthContextProvider';
// import SecureImage from '../SecureImage';
// import axios from 'axios';
// import ErrorBoundary from '../ErrorBoundary';
// import { IconClose, IconTick } from '../../assets/icons';
...
//   const [deleteIconDoc, setDeleteIconDoc] = useState(null);
...
//       window.open(fileURL); // Opens PDF in a new tab
//     } catch (error) {
...
//     // return paper.id !== id;
//     //});
//     const edited_doc_meta_type = document_meta_type?.filter((paper) => {
//       return paper.id !== id;
//     });
//     // const property_pdf = property_papers.find((paper) => {
//     //   return paper.id === id;
//     // });
//     const doc_meta_type_pdf = document_meta_type.find((paper) => {
//       return paper.id === id;
//     });
//     const edited_doc_meta_type_pdf = { ...doc_meta_type_pdf, active: false };
//     const edited_applicant = [...edited_doc_meta_type, edited_doc_meta_type_pdf];
//     let typeObj = { type: type };
//     const new_edited_applicant = await editFieldsById(
//       values?.applicants?.[activeIndex]?.applicant_details?.id,
//       'applicant',
//       {
//         document_meta: { ...document_meta, [typeObj?.type]: edited_applicant },
//       },
//       {
//         headers: {
//           Authorization: token,
...
//       },
//     );
//     const active_uploads = new_edited_applicant.document_meta?.[type]?.filter((data) => {
...
//   }
//   useEffect(() => {
//     if(configType === 'other_docs'){
//       if (openOtherDocDrawer) {
//         handleSelect(null);
//       }
//     }
//   }, [openOtherDocDrawer]);
...
//             <label
//               style={{ pointerEvents: configType === 'other_docs' ? 'none' : 'auto' }}
...
//           <div
//             className={`${
//               configType === 'other_docs' ? '' : 'flex'
//             } justify-start overflow-auto ${imageArrayBorder ? 'p-2 border border-[#D9D9D9] rounded-lg' : ''}`}
//           >
//             <div className={`${configType === 'other_docs' && 'flex-col'} flex  gap-2 my-2`}>
//               <div
...
//                     }
//                 }}
//                 className={` ${configType === 'other_docs' ? 'h-[50px] w-full' : 'h-[85px] w-[68px]'} rounded-lg border-x border-y  flex justify-center items-center`}
//               >
...
//                     multiple={true}
//                     name='files[]'
//                     // disabled={disabled || type == 'property_paper_photos'}  
//                     ref={inputRef}
//                     style={{ pointerEvents: configType === 'other_docs' ? 'none' : 'auto' }}
//                   />
//                 </button>
...
//               <div
//                 className={`${configType === 'other_docs' ? 'flex-col' : 'h-[85px]'} flex gap-2`}
//               >
...
//                         <div
//                           className={`${configType === 'other_docs' ? 'h-10 w-10' : 'h-full w-[68px] rounded-lg'} relative`}
//                         >
//                           {!lock ? (
...
//                             <div
//                               className={`${configType === 'other_docs' ? '' : 'rounded-lg'} absolute w-full h-full bg-black opacity-40`}
//                             ></div>
//                             <button
//                               className='absolute top-2/4 -translate-y-2/4 left-2/4 -translate-x-2/4 z-20'
//                               onClick={() => {
//                                 if (upload && upload.document_meta.mimetype === 'image/jpeg') {
//                                   setPreviewFile(index);
//                                   setShow(true);
//                                 } else {
//                                   setPreviewFile(index);
//                                   setShow(true);
//                                   openPdf(upload.document_fetch_url);
//                                   //window.open(upload.document_fetch_url, '_blank');
//                                 }
//                               }}
//                             >
//                               <svg
...
//                             {upload && upload.document_meta.mimetype === 'image/jpeg' ? (
...
//                         {configType === 'other_docs' && (
//                           <p className='text-base text-primary-black font-normal truncate w-[105px]'>
//                             {(() => {
//                               const allDocuments = Object.values(otherDocumentOptions).flat();
//                               const foundDocument = allDocuments.find(
//                                 (option) => option?.value === upload?.document_type_specific,
//                               );

//                               return foundDocument?.label || upload?.document_name;
//                             })()}
//                           </p>
//                         )}
//                       </div>
...
//             onClick={() => props.viewPdf && openPdf(pdf.document_fetch_url)}
//           >
...
//       {configType === 'other_docs' && openOtherDocDrawer ? (
//         <DynamicDrawer open={openOtherDocDrawer} setOpen={setOpenOtherDocDrawer}>
//           <div className='w-full flex flex-col'>
//             <div className='flex justify-between items-center pb-2 md:pb-4 border-b border-lighter-grey'>
//               <p className='font-semibold'>Choose a document</p>
//               <button
//                 onClick={() => {
...
//               >
//                 {otherDocumentOptions.map((option, index) => (
//                   <button
//                     key={option.value}
//                     id='random'
...
//       {configType === 'other_docs' && deleteIconDoc !== null ? (
//         <DynamicDrawer
//           open={deleteIconDoc !== null}
//           setOpen={() => setDeleteIconDoc(null)}
//           height='223px'
//         >
...
//             </div>
//             <div className=''>
//               <button onClick={() => setDeleteIconDoc(null)}>
//                 <IconClose />
//               </button>
//             </div>
//           </div>
...
//             <Button inputClasses='w-full h-[46px]' onClick={() => setDeleteIconDoc(null)}>
//               No, Keep it
//             </Button>
//             <Button
//               primary={true}
...
function PdfAndImageUpload({
  ...
  return (
    <div className='w-full'>
      ...
      {!files.length && !loader ? (
        ...
        >
          <div className='bg-white flex items-center justify-center w-full'>
            <label
              style={{ pointerEvents: configType === 'other_docs' ? 'none' : 'auto' }}
              className={`flex cursor-pointer flex-col w-full h-[72px] border-2 rounded-md ${message || errorMessage ? 'border-primary-red' : 'border-dashed border-stroke'
                } relative`}
            >
              <div className='flex flex-col items-center absolute top-2/4 -translate-y-2/4 left-2/4 -translate-x-2/4'>
                <svg
      ...
      {uploads && !pdf && !newLoader ? (
        <>
          <div
            className={`${configType === 'other_docs' ? '' : 'flex'
              } justify-start overflow-auto ${imageArrayBorder ? 'p-2 border border-[#D9D9D9] rounded-lg' : ''}`}
          >
            <div className={`${configType === 'other_docs' && 'flex-col'} flex  gap-2 my-2`}>
              <div

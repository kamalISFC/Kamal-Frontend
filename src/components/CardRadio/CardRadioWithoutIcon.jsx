...
const CardRadioWithoutIcon = memo(({ label, current, value, onChange, name, disabled }) => {
  const getThemes = () => {
    if (disabled) {
      if (current === value) {
  ...
  return (
    <div
      className={`flex flex-col gap-2 w-full cursor-pointer mb-[10px] ${
        disabled ? 'pointer-events-none' : 'cursor-pointer'
      }`}
    >
      <div
        className={`w-full border rounded-lg py-4 flex items-center justify-center cursor-pointer 
        ${getThemes()} transition-all duration-300 ease-out`}
        tabIndex={0}
        role='radio'
